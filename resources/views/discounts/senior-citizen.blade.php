<x-layout title="Senior Citizen Discounts">
    <!-- Page Header -->
    <div class="page-header">
        <h1 class="page-title">Senior Citizen Discount Transactions</h1>
    ...
    <div id="filters-section" class="bg-white p-4 shadow-sm border border-[var(--color-border-light)] mb-6 hidden">
        <form method="GET" action="{{ route('discounts.senior-citizen') }}" class="flex flex-wrap gap-3 items-end">
            ...
            <div class="form-group mb-0" style="min-width: 180px; flex: 1;">
                <label for="search" class="form-label text-xs mb-1">Senior Citizen Name</label>
            ...
            <tbody>
                @forelse($transactions as $transaction)
                    <tr>
                        ...
                        <td class="font-medium">{{ $transaction->sc_name }}</td>
                        <td>{{ $transaction->sc_birthdate->format('Y-m-d') }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="text-center py-8 text-[var(--color-text-secondary)]">
                            No senior citizen discount transactions found for the selected period.
                        </td>
                    </tr>
                @endforelse
            </tbody>
