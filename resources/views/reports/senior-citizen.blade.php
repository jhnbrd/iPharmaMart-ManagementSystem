<x-layout>
    <x-slot:title>Senior Citizen Report</x-slot:title>

    <div class="space-y-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold">Senior Citizen Discount Report</h1>
                <p class="text-sm text-gray-600 mt-1">Monthly report of senior citizen discount transactions</p>
            </div>
        ...
        <div class="bg-white p-6 border border-[var(--color-border-light)]">
            <form method="GET" action="{{ route('reports.senior-citizen') }}">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50 text-xs text-gray-600 uppercase">
                            ...
                            <th class="px-6 py-3 text-left">SC ID Number</th>
                            <th class="px-6 py-3 text-left">Senior Citizen Name</th>
                    ...
                    <tbody class="divide-y divide-[var(--color-border-light)]">
                        @forelse ($transactions as $transaction)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm font-medium">{{ $transaction->sc_id_number }}</td>
                                <td class="px-6 py-4 text-sm">{{ $transaction->sc_name }}</td>
                                <td class="px-6 py-4 text-sm">
                                    {{ \Carbon\Carbon::parse($transaction->sc_birthdate)->format('Y-m-d') }}
                                    <span class="text-xs text-gray-500">
                                        ({{ \Carbon\Carbon::parse($transaction->sc_birthdate)->age }} years)
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <p class="text-sm">No senior citizen transactions for this month</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
